...
*APIs* are RAML files that describe an API's structure, they can be referenced by an link:/mule-user-guide/v/3.8-m1/http-request-connector[HTTP Request Connector] to expose metadata to Studio.
...
* Through the global element of the link:/mule-user-guide/v/3.8-m1/http-request-connector[HTTP Request Connector], and the link to *Search RAML Exchange*.
...
[NOTE]
Some connectors may be viewable via Exchange but not yet installable through it, they soon will be. If the connector you need is not yet available for installation in Exchange, you can install it using the link:/mule-user-guide/v/3.8-m1/installing-connectors[install new software] menu.
...
. The example is now complete! You can deploy it to see what's its outcome, run it on link:/mule-user-guide/v/3.8-m1/studio-visual-debugger[debug mode] to examine what occurs with the Mule Message on each step, or modify it as needed.
...
. Now your app is now ready to link:/mule-fundamentals/v/3.8-m1/deploying-mule-applications[Deploy].

== Referencing a RAML File

When using the link:/mule-user-guide/v/3.8-m1/http-request-connector[HTTP Request Connector], you can reference a link:http://raml.org[RAML] file, which makes configuring the connector and the rest of your flow extremely easy. By referencing the RAML file, the connector offers you smart autocomplete options based on how the RAML file describes the available resources, methods and parameters. The metadata that the connector exposes can help you map it to other elements and reference its outputs elsewhere in the flow, specially with help from the link:/mule-user-guide/v/3.8-m1/data-weave[Transform Message Component].
...
When using the link:/mule-user-guide/v/3.8-m1/web-service-consumer[Web Service Consumer], you can reference a WSDL file, which makes configuring the connector and the rest of your flow extremely easy. By referencing the WSDL file, the connector offers you smart autocomplete options based on how the WSDL file describes the available resources, methods and parameters. The metadata that the connector exposes can help you map it to other elements and reference its outputs elsewhere in the flow, specially with help from the link:/mule-user-guide/v/3.8-m1/data-weave[Transform Message Component].
...
No users have permissions to submit items by default, not even organization administrators. If you are an organization administrator, it's easy to add a user to a role where others can submit items. After you log into link:https://anypoint.mulesoft.com/#/signin[Anypoint Platform], click *Access Management* from the left side navigation bar or the starting Anypoint Platform menu, then pick the *Roles* tab. This displays a table with a set of roles for various different tools, only three of which are relevant to the Exchange:
...
An *Exchange Contributor* submits her content to the Exchange, however this content remains unpublished and is only visible to her and to administrators until an administrator chooses to set the status of this contribution to *published*. She also can see all of the published content from others, but not edit or delete any of it.

An *Exchange Administrator* can publish her own and other user's unpublished content to the Exchange, where it is visible to everyone in the organization. She is able to see, edit, or delete any of the content from others, whether it's published or not.
...
Keep in mind that after submitting an item, it is added to the Exchange with an *unpublished* status, which makes it only visible to yourself and administrators. If you have the Exchange Administrator role, you can easily publish it by opening the Exchange entry through the *View Details* button and clicking the *Publish* button.
...
If you have a link:http://raml.org[RAML] definition file that describes your API, or if you have an API Portal to document it interactively, you can expose these to others in your organization using Exchange. If someone in your organization wants to connect to your API via the link:/mule-user-guide/v/3.8-m1/http-request-connector[HTTP Request Connector] in Anypoint Studio, referencing the API's RAML file exposes the API's metadata, making integration a lot easier. See the <<Referencing a RAML File>> section above for more information. If your API is registered in the Anypoint Platform for APIs, you can also link to its portal from the Exchange, which provides very versatile interactive tools for easing your internal user's engagement with it.
...
If you have a WSDL definition file that describes your API, you can expose it to others in your organization using Exchange. If someone in your organization wants to connect to your API via the link:/mule-user-guide/v/3.8-m1/web-service-consumer[Web Service Consumer] in Anypoint Studio, referencing the API's WSDL file exposes the API's metadata, making integration a lot easier. See the <<Referencing a WSDL File>> section above for more information.
...
Exchange 1.6 and newer supports business groups. Business groups are hierarchical in that groups created by administrators of an Anypoint Platform organization are visible across all groups, whereas subordinate groups are only visible to other groups, and those created by a line of business only are visible to just that line.
...
// The Business Groups feature in Exchange is a litle bit complex, the publish content is shared through the Business Groups using same kind of "hierarchy". For example, if some template is published in the master organization, all organization below are going to see this template in the content list. Also, the content list have new filters to search content: The 'Show content from' filter and the 'Scope' filter, the last one depends on the role that the user have in the organization that is selected in the navbar. Also, the content that is shown in the list by default depends in the organization or business group that is selected in the navbar.

== See Also

* Learn the different ways you can link:/mule-fundamentals/v/3.8-m1/deploying-mule-applications[Deploy] your app.
